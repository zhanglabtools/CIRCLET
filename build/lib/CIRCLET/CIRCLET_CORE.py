# -*- coding: utf-8 -*-
"""
Created on Sat Mar 10 21:25:48 2018
...
def CIRCLET(data, s, k=15, l=15, num_graphs=1, num_waypoints=150,
	...
	for graph_iter in range(num_graphs):
		...
		if branch:
			RNK,bp, diffdists, Y, branch,c_new = _splittobranches_v2(traj, traj[0], data, iter_l, dist, paths_l2l) 
		else:
			RNK,bp, diffdists, Y, branch,c_new = _splittobranches_v2(traj, traj[0], data, iter_l, dist, paths_l2l,branch=False)  
        
        #show diffdists
		# calculate weighed trajectory
		W_full = _weighting_scheme(voting_scheme, dist); mask = np.isnan(W_full); W_full[mask]=1/W_full.shape[0]
		if branch:
			W = W_full
		...
		t = traj[0, :]
		t1 =_compute_t1(c_new,RNK,W,traj) 
        
    
        #		W1_1=W[c_new==2,:]
        #		W1_1=W1_1/np.sum(W1_1,axis=0)
        #        
        #
        #		t1_1=np.sum(np.multiply(traj[c_new==2,:][:,RNK==2], W1_1[:,RNK==2]), axis=0)
        #		W1_2=W[c_new==3,:]
        #		W1_2=W1_2/np.sum(W1_2,axis=0)
        #
        #		t1_2=np.sum(np.multiply(traj[c_new==3,:][:,RNK==3], W1_2[:,RNK==3]), axis=0)
        #		t1=np.zeros(len(t))
        #        
        #		t1[RNK==2]=t1_1;t1[RNK==3]=t1_2
		t = [t, t1]
		...
		if branch:
			...
			RNK, bp,diffdists, Y, branch_v2,c_new = _splittobranches_v2(traj, traj[0], data, iter_l, dist,paths_l2l)
			branches.append(RNK)
			bas.append(Y)
...
def _compute_t1(c_new,RNK,W,traj):
...
#trajs=traj; t=traj[0]; landmarks=iter_l


def _splittobranches_v2(trajs, t, data, landmarks, dist, paths_l2l, branch=True):
	...
	diffdists_DF = pd.DataFrame(diffdists); X=diffdists_DF.corr()
	...
	#marks=np.argsort(evec2)

	#sns.heatmap(diffdists[:,marks][marks,:],cmap=cmap)
	clustering = linkage(X)
	k=2
	clu_label=fcluster(clustering, k, criterion='maxclust')

	#marks=np.argsort(clu_label)
	#sns.heatmap(X.values[:,marks][marks,:],cmap=cmap)
	evec2=clu_label*2-3
        
	#evec2 = spectral_clustering(diffdists,n_clusters=2, eigen_solver='arpack')

	#idx = np.argsort(kmeans.labels_)diffdists_DF = pd.DataFrame(diffdists)
	...
	c[b1_suspect[b1_sorted_inds[:min(neg_pos_number, len(b1_sorted_inds))]]] = 2
	c[b2_suspect[b2_sorted_inds[:min(neg_pos_number, len(b2_sorted_inds))]]] = 3
	pb=0
	...
	# 		break
	marks= np.where(c != 1)[0]
	c_new = c[np.where(c != 1)[0]]
	I = np.argmin(np.absolute(dist[marks,:]), axis=0)
	RNK = c_new[I]
	...
	A=np.sum(evec2==1)
	B=np.sum(evec2==-1)
	if (np.min([A,B])<len(evec2)/8) | (branch==False):
		branch=False; return RNK, pb,diffdists, Y, branch, c_new
	else:
		return RNK, pb,diffdists, Y, branch,c_new
